...
in with pkgs; rec {
  ...
  bootstrapTools =
    ...
    import ./bootstrap-tools {
      inherit (lib.traceVal stdenv.targetPlatform) system; # Used to determine where to build
      inherit bootstrapFiles extraAttrs;
    }
    else if (stdenv.hostPlatform.libc == "musl") then
    import ./bootstrap-tools-musl {
      inherit (stdenv.targetPlatform) system; # Used to determine where to build
      inherit bootstrapFiles extraAttrs;
    }
